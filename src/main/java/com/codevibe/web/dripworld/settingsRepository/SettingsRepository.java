package com.codevibe.web.dripworld.settingsRepository;
...
public class SettingsRepository {
    public List<SettingsEntity> findAll() {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            return session.createQuery("SELECT a FROM SettingsEntity a", SettingsEntity.class)
                    .getResultList();
        }
    }
    ...
    public Optional<SettingsEntity> findByName(SettingType name) {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            return session.createQuery("SELECT a FROM SettingsEntity a WHERE a.name=:name", SettingsEntity.class)
